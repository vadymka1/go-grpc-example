...
func init() {
  ...
  rootCmd.PersistentFlags().StringVar(&cfgFile, "config", "", "config file (default is $HOME/.go-grpc.yaml)")
...
func initConfig() {
  if cfgFile != "" {
    ...
    // Search config in home directory with name ".go-grpc" (without extension).
    viper.AddConfigPath(home)
    viper.SetConfigName(".go-grpc")
  }
